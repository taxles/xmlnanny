...
static void texmlmateModuleElementReportFired(xsltTransformContextPtr ctxt,
											  xmlNodePtr inputNode,
											  xmlNodePtr sheetNode,
											  xsltStylePreCompPtr comp)
{
...
static void texmlmateModuleElementAssertFired(xsltTransformContextPtr ctxt,
											  xmlNodePtr inputNode,
											  xmlNodePtr sheetNode,
											  xsltStylePreCompPtr comp)
{
...
static void *texmlmateModuleInit(xsltTransformContextPtr ctxt, const xmlChar *URI)
{	
	xsltRegisterExtElement(ctxt,(const xmlChar *)"assert-fired", URI, 
						   (xsltTransformFunction)texmlmateModuleElementAssertFired);
	xsltRegisterExtElement(ctxt,(const xmlChar *)"report-fired", URI, 
						   (xsltTransformFunction)texmlmateModuleElementReportFired);
	return NULL;
}


static void *texmlmateModuleShutdown(xsltTransformContextPtr ctxt,
									 const xmlChar *URI,
									 void *data)
{
	return NULL;
...
- (void)loadMetaStylesheet;
{
	...
	xsltRegisterExtModule((const xmlChar *)"http://scan.dalo.us/texmlmate",
						  (xsltExtInitFunction)texmlmateModuleInit,
						  (xsltExtShutdownFunction)texmlmateModuleShutdown);
	...
	NSString *path = [[NSBundle mainBundle] pathForResource:@"sch-custom" ofType:@"xsl"];
